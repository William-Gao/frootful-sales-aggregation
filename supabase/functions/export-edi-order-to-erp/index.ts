...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { ediOrderId, orderData }: { ediOrderId: string; orderData: OrderData } = await req.json();

    if (!ediOrderId || !orderData || !orderData.customerNumber || !orderData.items || orderData.items.length === 0) {
      return new Response(
        JSON.stringify({ 
          success: false, 
          error: 'Invalid request. EDI order ID, customer number and items are required.' 
        }),
        {
          status: 400,
          headers: {
    ...
    console.log('Exporting EDI order to ERP:', ediOrderId);
    ...
    // Verify the EDI order belongs to the user
    const { data: ediOrder, error: fetchError } = await supabase
      .from('edi_orders')
      .select('*')
      .eq('id', ediOrderId)
      .eq('user_id', userId)
    ...
    if (fetchError || !ediOrder) {
      return new Response(
        JSON.stringify({ 
          success: false, 
          error: 'EDI order not found or access denied.' 
        }),
        {
          status: 404,
          headers: {
    ...
    // Step 3: Update EDI order with ERP information
    console.log('Updating EDI order with ERP information...');
    ...
    if (updateError) {
      console.warn('Failed to update EDI order status:', updateError);
    ...
    console.log('EDI order export completed successfully!');

    const successMessage = orderData.requestedDeliveryDate 
      ? `Successfully created order #${orderNumber} from EDI document with ${addedItems.length} items and delivery date ${orderData.requestedDeliveryDate}`
      : `Successfully created order #${orderNumber} from EDI document with ${addedItems.length} items`;
    ...
    console.error('Error exporting EDI order to ERP:', error);
    return new Response(
      JSON.stringify({
        success: false,
        error: error instanceof Error ? error.message : 'Unknown error occurred while exporting EDI order'
      }),
      {
        headers: {
          'Content-Type': 'application/json',
...
// Helper functions (reused from other export functions)
